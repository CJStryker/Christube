...
$legacyPath = mediaLegacyUrl((string)($video['file_path'] ?? ''));
if ($playbackUrl === '' && $legacyPath !== '') {
    // Preserve compatibility with videos created before the media-asset pipeline.
    $playbackUrl = $legacyPath;
}
$playbackMime = $playbackUrl !== '' ? ($legacyPath !== '' ? mediaMimeForPath($legacyPath) : 'video/mp4') : '';
...
<?php if($playbackUrl): ?><video id="watchPlayer" controls playsinline preload="metadata" src="<?php echo e($playbackUrl); ?>" type="<?php echo e($playbackMime); ?>" poster="<?php echo e($thumbUrl); ?>"></video><?php elseif($legacyPath && is_file(__DIR__ . '/' . ltrim($legacyPath, '/'))): ?><video id="watchPlayer" controls playsinline preload="metadata" src="<?php echo e($legacyPath); ?>" type="<?php echo e($playbackMime); ?>"></video><?php else: ?><p class="meta">Playback not ready. <?php echo e((string)$video['processing_error']); ?></p><?php endif; ?>
