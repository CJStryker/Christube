...
foreach ($jobs as $job) {
    ...
    try {
        ...
        // Configure binaries explicitly for Windows/containers; retain PATH lookup as fallback.
        $mediaCfg = mediaConfig();
        $ffmpeg = $mediaCfg['ffmpeg'];
        $ffprobe = $mediaCfg['ffprobe'];
        if ($ffmpeg === '' && DIRECTORY_SEPARATOR !== '\\') $ffmpeg = trim((string)shell_exec('command -v ffmpeg 2>/dev/null'));
        if ($ffprobe === '' && DIRECTORY_SEPARATOR !== '\\') $ffprobe = trim((string)shell_exec('command -v ffprobe 2>/dev/null'));

        if ($ffmpeg !== '' && (is_file($ffmpeg) || DIRECTORY_SEPARATOR !== '\\')) {
            $cmd = sprintf('%s -y -i %s -vf "scale=1280:-2" -c:v libx264 -preset veryfast -crf 23 -c:a aac -movflags +faststart %s 2>&1', escapeshellarg($ffmpeg), escapeshellarg($sourceAbs), escapeshellarg($playAbs));
            exec($cmd, $out, $code);
            if ($code !== 0 || !is_file($playAbs) || filesize($playAbs) < 1) {
                throw new RuntimeException('FFmpeg could not convert this video. ' . substr(implode(' ', $out ?? []), -180));
            }

            $thumbCmd = sprintf('%s -y -ss 00:00:01 -i %s -vframes 1 -q:v 2 %s 2>&1', escapeshellarg($ffmpeg), escapeshellarg($sourceAbs), escapeshellarg($thumbAbs));
            exec($thumbCmd);
        } else {
            // Without FFmpeg, only browser-native formats can be played directly.
            $sourceMime = (string)($source['mime_type'] ?? '');
            if (!in_array($sourceMime, ['video/mp4', 'video/webm', 'video/ogg'], true)) {
                throw new RuntimeException('FFmpeg is required to convert this video to a browser-playable MP4.');
            }
            if (!copy($sourceAbs, $playAbs)) {
                throw new RuntimeException('Unable to create playback file.');
            }
        }
        ...
        $height = null;
        if ($ffprobe !== '') {
            $probe = sprintf('%s -v quiet -print_format json -show_streams -show_format %s', escapeshellarg($ffprobe), escapeshellarg($sourceAbs));
        ...
        $nextStatus = ((int)$job['attempts'] + 1 >= (int)$job['max_attempts']) ? 'failed' : 'queued';
        $pdo->prepare("UPDATE media_jobs SET status=?, run_after = DATE_ADD(NOW(), INTERVAL 2 MINUTE), last_error=? WHERE id=?")
            ->execute([$nextStatus, substr($e->getMessage(), 0, 250), $jobId]);
